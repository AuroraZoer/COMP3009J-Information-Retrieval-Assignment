...
def save_results(score: dict[str, dict[str, float]]) -> None:
    ...
    Note: Only the top 20 results are saved for each query
    I chose to save the top 20 results because evaluations showed that this approach yielded relatively higher scores.

    :param score: dictionary of scores
    :return: None
    """
    with open(f'{UCD_NUMBER}-small.results', encoding='utf-8', mode='w') as f:
        for qID, scores in score.items():
            sorted_scores = sorted(scores.items(), key=lambda item: item[1], reverse=True)
            for i, (docID, score) in enumerate(sorted_scores[:20]):
